...
type DefenseProfile = ReturnType<typeof buildDefenseProfile>;

export default function OperatorPage() {
    const [state, setState] = useState<OperatorState | null>(null);
    const [defendProfile, setDefendProfile] = useState<DefenseProfile | null>(null);
    const { avatarUrl, configured, displayName, user } = useSupabaseSession();

    useEffect(() => {
        const timer = window.setTimeout(() => {
            setState(createOperatorState());
            setDefendProfile(buildDefenseProfile());
        }, 0);
            ...
            <div className="relative z-10 mx-auto flex w-full max-w-7xl flex-col gap-6">
                ...
                {defendProfile && (
                    <PanelCard
                        variant="darkOps"
                        accent="emerald"
                        className="border-emerald-300/12 bg-[radial-gradient(circle_at_top_right,rgba(16,185,129,0.11),transparent_34%),linear-gradient(180deg,rgba(3,16,13,0.92),rgba(3,7,18,0.80))] p-6"
                    >
                        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
                            <div>
                                <AppBadge variant="emerald">Defense Analytics</AppBadge>
                                <h2 className="mt-4 text-2xl font-black text-white">Defensive operator profile</h2>
                            </div>
                            <div className="flex items-center gap-3">
                                <AppBadge variant="emerald">{defendProfile.overall.level}</AppBadge>
                                <AppButton href="/defend/defense-profile" variant="nexus">
                                    Full report
                                </AppButton>
                            </div>
                        </div>
                        ...
                        <div className="grid gap-4 sm:grid-cols-4">
                            <DefendStatTile
                                label="Overall score"
                                value={`${defendProfile.overall.score}%`}
                                sub={defendProfile.overall.level}
                            />
                            <DefendStatTile
                                label="Phishing accuracy"
                                value={`${defendProfile.phishing.accuracy}%`}
                                sub={`${defendProfile.phishing.total} analysed`}
                            />
                            <DefendStatTile
                                label="Security check"
                                value={`${defendProfile.securityCheck.percent}%`}
                                sub={`${defendProfile.securityCheck.answered} answered`}
                            />
                            <DefendStatTile
                                label="SOC incidents"
                                value={defendProfile.soc.incidents}
                                sub={`${defendProfile.soc.highSeverity} high severity`}
                            />
                        </div>

                        {defendProfile.overall.recommendations.length > 0 && (
                            <div className="mt-5 grid gap-3 md:grid-cols-2">
                                {defendProfile.overall.recommendations.slice(0, 2).map((rec, i) => (
                                    <div
                                        key={i}
                                        className="rounded-2xl border border-emerald-300/15 bg-emerald-300/[0.04] p-4 text-sm leading-6 text-slate-300"
                                    >
...
function DefendStatTile({ label, value, sub }: { label: string; value: string | number; sub: string }) {
    return (
        <div className="rounded-2xl border border-emerald-300/14 bg-emerald-300/[0.04] p-4">
            <p className="font-mono text-[10px] font-black uppercase tracking-[0.24em] text-slate-500">{label}</p>
            <p className="mt-2 text-2xl font-black text-white">{value}</p>
            <p className="mt-1 font-mono text-[10px] text-emerald-300/70">{sub}</p>
        </div>
    );
}
