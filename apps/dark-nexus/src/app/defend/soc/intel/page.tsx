...
function getSender(alert: Record<string, unknown>) {
    return (alert.artifact as Record<string, unknown>)?.sender as string || alert.sender as string || "Unknown sender";
}

function getTechniques(alert: Record<string, unknown>) {
    return [alert.mitre as string, ...((alert.mitreTechniques as string[]) || [])].filter(Boolean);
}
...
export default function SocIntelPage() {
    ...
    const intel = useMemo(() => {
        ...
        allAlerts.forEach((alert) => {
            ...
            getTechniques(alert).forEach((technique) => incrementCount(mitreCounts, technique));
            ((alert.signals as string[]) || []).forEach((signal) => incrementCount(signalCounts, signal));
            ...
            senderRisk.set(sender, {
                ...current,
                high: current.high + (alert.severity === "Critical" || alert.severity === "High" ? 1 : 0),
        ...
        return {
            ...
            suspiciousSenders: sortCounts(senderCounts)
                .map((item) => ({ ...item, ...(senderRisk.get(item.label) || {}) }))
        ...
        <SocLayout>
            ...
            <section className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
                        ...
                        <div className="flex flex-wrap gap-3">
                            {intel.mitreTechniques.map((technique) => (
                                ...
                                >
                                    ...
                                    <span className="font-mono text-xs text-slate-500">x{technique.count}</span>
                                </div>
                            ))}
                        </div>
                    ) : (
                ...
                <PanelCard variant="darkOps" accent="amber">
                    <div className="mb-5 flex items-center justify-between gap-4">
                        <p className="font-mono text-xs uppercase tracking-[0.28em] text-amber-200">Top categories</p>
            ...
            <section className="mt-6 grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
                <PanelCard variant="darkOps" accent="blue">
                    <div className="mb-5 flex items-center justify-between gap-4">
                        <p className="font-mono text-xs uppercase tracking-[0.28em] text-blue-200">Suspicious senders</p>
                ...
                <PanelCard variant="darkOps" accent="danger">
                    <div className="mb-5 flex items-center gap-2 text-red-200">
                        ...
                        <p className="font-mono text-xs uppercase tracking-[0.28em]">High severity alerts</p>
                    </div>
                    <div className="space-y-3">
                        {intel.highSeverityAlerts.slice(0, 5).map((alert) => (
                            <div key={alert.id} className="rounded-xl border border-red-300/12 bg-red-400/[0.045] p-3">
            ...
            <section className="mt-6 grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
                <PanelCard variant="darkOps" accent="blue">
                    <p className="mb-5 font-mono text-xs uppercase tracking-[0.28em] text-blue-200">Top senders</p>
                    <div className="space-y-3">
                ...
                <PanelCard variant="darkOps" accent="blue">
                    <div className="mb-5 flex items-center justify-between gap-4">
                        <p className="font-mono text-xs uppercase tracking-[0.28em] text-blue-200">Signal patterns</p>
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                        {intel.signalPatterns.map((signal) => (
                            ...
                            >
                                <p className="text-sm font-bold leading-6 text-slate-200">{signal.label}</p>
...
function IntelStat({ icon, label, value }: { icon: React.ElementType; label: string; value: string | number }) {
    return (
        <PanelCard variant="darkOps" accent="blue">
...
function RankRow({ label, value, tone = "blue" }: { label: string; value: string; tone?: "blue" | "amber" }) {
    const toneClass =
        tone === "amber"
