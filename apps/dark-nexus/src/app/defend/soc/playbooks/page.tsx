...
import { socPlaybooks } from "@/defend/data/socPlaybooks";
...
function alertText(alert: Record<string, unknown>) {
    return [
        alert.title,
        alert.category,
        alert.description,
        alert.mitre,
        ...((alert.mitreTechniques as string[] | undefined) || []),
        ...((alert.signals as string[] | undefined) || []),
        ...((alert.indicators as Array<{ value: string }> | undefined) || []).map((indicator) => indicator.value),
    ]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
...
function getMatchCount(playbook: Record<string, unknown>, alerts: Record<string, unknown>[]) {
    const playbookTerms = [
        playbook.category,
        playbook.title,
        ...((playbook.recommendedFor as string[] | undefined) || []),
        ...((playbook.mitreTechniques as string[] | undefined) || []),
    ]
        .map(normalize)
        .filter(Boolean);
...
export default function SocPlaybooksPage() {
    ...
    const playbooksWithMatches = useMemo(
        () => socPlaybooks.map((playbook) => ({ ...playbook, matchCount: getMatchCount(playbook, allAlerts) })),
        [allAlerts]
    );

    const filteredPlaybooks = useMemo(
        () => playbooksWithMatches.filter((playbook) => activeFilter === "All" || playbook.category === activeFilter),
    ...
    return (
        <SocLayout>
            ...
            <PanelCard variant="darkOps" accent="blue" className="mb-5 p-4">
            ...
            <section className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
                <div className="grid gap-4">
                    {filteredPlaybooks.length ? (
                        filteredPlaybooks.map((playbook) => {
                            const Icon = categoryIcons[playbook.category as keyof typeof categoryIcons] || Layers;
                            ...
                            return (
                                ...
                                >
                                    <PanelCard
                                        ...
                                        hover
                                        className={["p-4 transition", active ? "border-blue-300/28 bg-blue-400/[0.07]" : ""].join(" ")}
                                    >
                                        <div className="flex items-start justify-between gap-4">
                                            <div className="flex min-w-0 gap-3">
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <h2 className="text-xl font-black text-white">{playbook.title}</h2>
                                        ...
                                        <div className="mt-4 flex flex-wrap gap-2">
                                            ...
                                            <AppBadge variant="default">
                                                {playbook.matchCount} matching alert{playbook.matchCount === 1 ? "" : "s"}
                                            </AppBadge>
                                        </div>
                                    </PanelCard>
                                </button>
                ...
                {selectedPlaybook && (
                    <aside className="xl:sticky xl:top-8 xl:self-start">
                        <PanelCard variant="darkOpsHero" accent={severityVariant(selectedPlaybook.severity)} className="p-5">
                            <div className="mb-5 flex flex-wrap items-start justify-between gap-4">
                                <div>
                                    ...
                                    <h2 className="text-3xl font-black text-white">{selectedPlaybook.title}</h2>
                                        ...
                                        >
                                            <div className="flex gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <AppBadge variant="blue" className="mt-3">{step.action}</AppBadge>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                            ...
                            <DetailSection title="Report Template Preview">
                                <div className="rounded-xl border border-slate-300/10 bg-black/30 p-4">
                                    <div className="mb-3 flex items-center gap-2 text-blue-200">
                                        ...
                                        <p className="font-mono text-[10px] uppercase tracking-[0.22em]">template.preview</p>
                                    </div>
                                    <p className="text-sm leading-6 text-slate-300">{selectedPlaybook.reportTemplate}</p>
                                </div>
                            </DetailSection>
...
function DetailSection({ title, children }: { title: string; children: React.ReactNode }) {
    return (
        <section className="mt-5">
            <p className="mb-3 font-mono text-xs uppercase tracking-[0.25em] text-blue-200">{title}</p>
            {children}
        </section>
    );
}
