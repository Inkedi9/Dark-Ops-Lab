...
import { getIncidents } from "@/defend/lib/defend/incidentService";
import SocIncidentReport from "@/defend/components/soc/reports/SocIncidentReport";
            ...
            <section className="grid gap-5">
                {incidents.length > 0 ? (
                    ...
                    <EmptyState
                        ...
                        actionTo="/defend/soc/alerts"
                    />
                )}
            </section>

            <PanelCard variant="darkOpsHero" accent="amber" className="mt-6">
                <div className="flex items-start gap-4">
                    <AlertTriangle className="mt-1 h-6 w-6 text-amber-200" />
                    <div>
                        <p className="font-mono text-xs uppercase tracking-[0.35em] text-amber-200">Mock mode</p>
...
function MockReportCard({ report }: { report: { id: string; title: string; severity: string; status: string; type: string; summary: string; timeline: string[] } }) {
    return (
        <PanelCard variant="darkOps" accent={severityVariant(report.severity)} hover>
            <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
                <div>
                    <div className="mb-4 flex flex-wrap gap-2">
                        <AppBadge variant={severityVariant(report.severity)}>{report.severity}</AppBadge>
                        <AppBadge variant={statusVariant(report.status)}>{report.status}</AppBadge>
                        <AppBadge variant="blue">{report.type}</AppBadge>
                    </div>
                    <p className="font-mono text-xs uppercase tracking-[0.3em] text-slate-500">{report.id}</p>
                    <h2 className="mt-2 text-2xl font-black text-white">{report.title}</h2>
                ...
                <div className="rounded-xl border border-slate-300/10 bg-slate-400/[0.035] p-4">
                    <p className="mb-3 font-mono text-[10px] uppercase tracking-[0.25em] text-blue-200">Case timeline</p>
