...
import { getFinalStats } from "@/defend/utils/scoring";
...
import { PhishLayout } from "@/defend/components/ui/PhishLayout";
...
import { PhishMetric } from "@/defend/components/ui/PhishMetric";
import { getDarkProfile } from "@/defend/lib/defend/defendProgressService";

type DarkProfile = { rank: string; level: number | string; xp: number | string };

function getUserLevel(accuracy: number) {
...
export default function Results() {
    ...
    const reasoningCount = storedResults.filter((result) => result.analystReasoning?.trim()).length;
                    ...
                    {storedResults.length === 0 ? (
                        ...
                        <div className="mt-6 space-y-6">
                            <PhishPanel variant="glow">
                                ...
                                <h2 className="mt-3 text-4xl font-black text-white">{userLevel}</h2>
                            ...
                            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                                ...
                                {profile && (
                                    <PhishMetric label="DarkOps XP" value={profile.xp} tone="green" />
                                )}
                            ...
                            <div className="grid gap-6 xl:grid-cols-2">
                                <PhishPanel variant="card">
                                    <h3 className="mb-4 text-xl font-black text-white">Detection Signals</h3>
                                ...
                                <PhishPanel variant="card">
                                    <h3 className="mb-4 text-xl font-black text-white">Decision Quality</h3>
                                    <ResponsiveContainer width="100%" height={260}>
                                        <PieChart>
                                            <Pie
                                                data={chartData}
                                ...
                                <PhishPanel variant="card">
                                    ...
                                    <h3 className="mt-2 text-2xl font-black text-white">Decision Discipline</h3>
                            ...
                            <PhishPanel variant="card">
                                ...
                                <h3 className="mt-2 text-2xl font-black text-white">Skill Breakdown</h3>
                                <div className="mt-5 grid gap-3 md:grid-cols-5">
                                    {pathBreakdown.map((path) => (
                                        <div key={path.id} className="rounded-2xl border border-blue-400/15 bg-black/25 p-4">
                                            ...
                                            <p className="mt-3 text-3xl font-black text-white">{path.accuracy}%</p>
                            ...
                            <PhishPanel>
                                ...
                                <h3 className="mt-2 text-2xl font-black text-white">Analyst Feedback</h3>
...
function WorkflowTile({ label, value, tone = "blue" }: { label: string; value: string | number; tone?: "blue" | "green" | "amber" | "red" | "slate" }) {
    const tones = {
