...
import { securityCheckQuestions } from "@/defend/data/securityCheckQuestions";
...
import { recordSecurityCheckCompleted } from "@/defend/lib/defend/defendProgressEvents";

type AnswerOption = { label: string; score: number; risk: string; recommendation: string };
type CategoryScore = { category: string; answered: number; total: number; score: number; maxScore: number; percent: number };
...
function safeLoadAnswers(): Record<string, AnswerOption> {
    try {
        ...
        return parsed && typeof parsed === "object" && !Array.isArray(parsed) ? (parsed as Record<string, AnswerOption>) : {};
    } catch {
        return {};
    }
}

function answerScore(answer: AnswerOption | undefined) {
    return Number(answer?.score) || 0;
}

function getRiskProfile(percent: number) {
    if (percent >= 85) {
        return {
...
function getCategoryScores(answers: Record<string, AnswerOption>) {
    const categories = securityCheckQuestions.reduce<Record<string, CategoryScore>>((acc, question) => {
...
function getWeakAreas(categoryScores: CategoryScore[]) {
    return categoryScores
        .filter((category) => category.answered > 0 && category.percent < 70)
        .sort((a, b) => a.percent - b.percent);
}

function getRecommendations(answers: Record<string, AnswerOption>) {
...
export default function SecurityCheckPage() {
    ...
    function handleAnswer(question: typeof securityCheckQuestions[0], answer: AnswerOption) {
        ...
        <main className="relative min-h-screen overflow-hidden bg-[#05070A] px-5 py-8 text-slate-100">
            ...
            <section className="relative z-10 mx-auto max-w-7xl">
                ...
                <SectionHeader
                    ...
                    actions={<AppBadge variant="blue">local diagnostic</AppBadge>}
                />
                ...
                <section className="grid gap-5 lg:grid-cols-[1fr_360px]">
                    <div className={spacing.stack}>
                        {securityCheckQuestions.map((question, index) => {
                            ...
                            const Icon = categoryIcons[question.category as keyof typeof categoryIcons] ?? ShieldCheck;
                                    ...
                                    <div className="flex flex-wrap items-start justify-between gap-4">
                                        <div className="flex min-w-0 gap-3">
                                            ...
                                            <div className="min-w-0">
                                                ...
                                                <h2 className="mt-2 text-lg font-black text-white">{question.question}</h2>
                        ...
                        {isComplete && (
                            <PanelCard variant="darkOpsHero" accent="emerald" className="p-5">
                    ...
                    <aside className="space-y-5 lg:sticky lg:top-8 lg:self-start">
                        <PanelCard variant="darkOpsHero" accent={riskProfile.variant} className="p-5">
                            <p className="font-mono text-[10px] uppercase tracking-[0.25em] text-slate-500">
                                posture.score
                            </p>
                            <p className={`mt-3 text-6xl font-black ${riskProfile.tone}`}>{percent}%</p>
                            <div className="mt-4">
                        ...
                        <PanelCard variant="darkOps" accent={weakAreas.length ? "amber" : "emerald"} className="p-4">
...
function ScoreRow({ category }: { category: CategoryScore }) {
