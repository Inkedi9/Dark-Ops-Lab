...
const items = [
    { href: "/defend/soc", label: "Overview", icon: Shield },
    { href: "/defend/soc/alerts", label: "Alerts", icon: Siren },
    { href: "/defend/soc/intel", label: "Threat Intel", icon: Radar },
    { href: "/defend/soc/playbooks", label: "Playbooks", icon: Network },
];
    ...
    return (
        <aside className="relative flex h-screen w-[290px] flex-col overflow-hidden border-r border-blue-300/10 bg-[#050816]/96 backdrop-blur-2xl">
            ...
            <div className="relative z-10 flex items-center gap-3 border-b border-white/[0.06] px-6 py-6">
                ...
                <div>
                    <p className="font-mono text-[10px] uppercase tracking-[0.35em] text-blue-300">DarkDefend</p>
                    <h2 className="mt-1 text-xl font-black text-white">SOC Core</h2>
                </div>
            </div>
            ...
            <div className="relative z-10 flex-1 px-4 py-5">
                <div className="space-y-2">
                    {items.map((item) => {
                        const active = item.href === "/defend/soc"
                            ? pathname === "/defend/soc"
                        ...
                        return (
                            ...
                            >
                                <div className={cn("grid h-10 w-10 place-items-center rounded-xl border transition", active ? "border-blue-300/20 bg-blue-400/[0.08]" : "border-white/[0.06] bg-black/30")}>
                                    <Icon className="h-5 w-5" />
                                </div>
                                <p className="font-mono text-xs uppercase tracking-[0.18em]">{item.label}</p>
                            </Link>
                        );
                    })}
                </div>
            ...
            <div className="relative z-10 border-t border-white/[0.06] p-5">
                <div className="rounded-2xl border border-red-300/14 bg-red-400/[0.05] p-4">
                    <div className="mb-3 flex items-center justify-between">
                        <p className="font-mono text-[10px] uppercase tracking-[0.28em] text-red-200">Threat Level</p>
                    ...
                    <p className="mt-3 text-xs text-slate-400">Elevated hostile activity detected.</p>
                </div>
            </div>
        </aside>
    );
