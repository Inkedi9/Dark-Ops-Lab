...
export function getDefendTone(tone: DefendToneName | string = "slate") {
    const normalized = (toneAliases as Record<string, string>)[tone] ?? tone;
    return (defendTones as Record<string, typeof defendTones.slate>)[normalized] ?? defendTones.slate;
}
